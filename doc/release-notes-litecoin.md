Trannychain Core version *0.15.1* is now available from:

  <https://download.trannychain.org/trannychain-0.15.1/>
...
A number of changes to the way Trannychain Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 trannies per kB (0.00001 TRANNY). This is to prepare for dropping the minimum transaction fee to 0.1 trannies per kB (0.0001 TRANNY) in the next release.
